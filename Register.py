...
# #        ---- left image ---
       ...
       #---register frame---
        ...
        uname=Label(frame1,text="Username",font=("times new roman",15,"bold"),bg="white",fg="gray").place(x=370,y=170)
        self.txt_uname = Entry(frame1,font=("times new roman",15),bg="lightgray")
        self.txt_uname.place(x=370,y=200,width=250)
        ...
        enroll=Label(frame1,text="Enrollment Number",font=("times new roman",15,"bold"),bg="white",fg="gray").place(x=50,y=310)
        self.txt_enroll = Entry(frame1,font=("times new roman",15),bg="lightgray")
        self.txt_enroll.place(x=50,y=340,width=250)
    ...
    def register_data(self):
        if self.txt_fname.get()=="" or self.txt_contact.get()=="" or self.txt_uname.get()=="" or self.cmb_quest.get()=="select" or self.cmb_quest2.get()=="select" or self.txt_enroll.get()=="" or self.txt_cpassword.get()=="":
            ...
            try:
                ...
                cur=con.cursor()
                cur.execute("select * from student where uname=%s",self.txt_uname.get())
                row=cur.fetchone()
                #print(row)
                if row!=None:
                  messagebox.showerror("Error","User Already Exist,Please Try With Another Username",parent=self.root)
                else:
                    cur.execute("insert into student (f_name,l_name,contact,uname,branch,semester,enroll) values(%s,%s,%s,%s,%s,%s,%s)",
                                    (self.txt_fname.get(),
                                     self.txt_lname.get(),
